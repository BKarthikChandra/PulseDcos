AIRS (Artifact Ingestion & Retrieval System) — Current Architecture (v0.4)
...
The current implementation focuses on PDF-based documents. However, the system is intentionally designed to be artifact agnostic and can ingest other artifact types such as source code and logs using the same state-driven ingestion and retrieval model without changing core correctness guarantees.w
